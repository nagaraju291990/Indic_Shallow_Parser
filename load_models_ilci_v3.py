...
def load_online_model(lang):
    ...
    return {'pos':tf.saved_model.load('models/M2/'+lang+'_pos')}
...
vocab_file = {
        'ass':'models/M2/ass_pos/assets/vocab.txt',
        'ban':'models/M2/ban_pos/assets/vocab.txt',
        'bodo':'models/M2/bodo_pos/assets/vocab.txt',
        'guj':'models/M2/guj_pos/assets/vocab.txt',
        'hin':'models/M2/hin_pos/assets/vocab.txt',
        'kan':'models/M2/kan_pos/assets/vocab.txt',
        'kon':'models/M2/kon_pos/assets/vocab.txt',        
        'mal':'models/M2/mal_pos/assets/vocab.txt',
        'mar':'models/M2/mar_pos/assets/vocab.txt',        
        'nep':'models/M2/nep_pos/assets/vocab.txt',
        'ori':'models/M2/ori_pos/assets/vocab.txt',        
        'pun':'models/M2/pun_pos/assets/vocab.txt',
        'tam':'models/M2/tam_pos/assets/vocab.txt',        
        'tel':'models/M2/tel_pos/assets/vocab.txt',
        'urd':'models/M2/urd_pos/assets/vocab.txt',  
        #'all':'models/M2/all_pos/assets/vocab.txt', 
        #'all2':'models/M2/all2_pos/assets/vocab.txt', 
        #'allND':'models/M2/allND_pos/assets/vocab.txt', 
        #'indoaryan':'models/M2/indoaryan_pos/assets/vocab.txt', 
        #'indoaryan2':'models/M2/indoaryan2_pos/assets/vocab.txt', 
        #'dravidian':'models/M2/dravidian_pos/assets/vocab.txt', 
        #'dravidian2':'models/M2/dravidian2_pos/assets/vocab.txt', 
        #'dravidianplus':'models/M2/dravidianplus_pos/assets/vocab.txt', 
        }
tokenizer = {
...
def predict_common(text, model, label, language='hin'):
    ...
    for sample in a4:
        ...
        for _idx in range(len(tasks[language])):
            label=labels[language][tasks[language][_idx]]
            #print (tasks[language][_idx],label)
            #print ([i for i in preds[_idx][0][s].numpy()])
            class_index = [label[i] for i in tf.math.argmax(preds[s], 1).numpy()]
            ...
            if len(text[s].split()) == len(pred_labels):
                output[tasks[language][_idx]] = [text[s].split()[i]+'$%:%$'+pred_labels[i] for i in range(len(text[s].split()))]
            else:
                output[tasks[language][_idx]] = [text[s].split()[i]+'$%:%$'+' ' for i in range(len(text[s].split()))]
...
def dasdapredict_common(text, model, label, language='hin'):
...
def shallow_parseisdas(text, language='hin', mode='ssf'):
...
def shallow_parse(text, language='hin', mode='ssf'):
    ...
    if len(_sentences)>0:
        ...
        p_all = _all(_sentences, language)
        #print (json.dumps(p_all))
        #print (p_all)
        _sid = 0
        for sentence in _sentences:
            '''
            chunk_ = p_all[sentence][tasks[language][7]]
            pos_ = p_all[sentence][tasks[language][0]]
            morphpos = p_all[sentence][tasks[language][1]]
            morphgender = p_all[sentence][tasks[language][2]]
            morphnumber = p_all[sentence][tasks[language][3]]
            morphperson = p_all[sentence][tasks[language][4]]
            morphcase = p_all[sentence][tasks[language][5]]
            morphvib = p_all[sentence][tasks[language][6]]
            '''
