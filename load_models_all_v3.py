...
models_path = {
        ...
        'mar':{'all':'models/mr_all'},
        'kan':{'all':'models/ka_all'},
        'mal':{'all':'models/ml_all'},
        'ban':{'all':'models/bn_all'},
        'tel':{'all':'models/te_all'},
        'urd':{'all':'models/ud_all'},
        }




def load_online_model(language, modelname='all'):
    print ('Language :', language)
    model_path = models_path[language][modelname]
...
def __predict_common(text, model, label, language='hin'):
...
def __shallow_parse(text, language='hin', mode='ssf'):
...
for line in codecs.open('nov_12_2022_history.json','r'):
    tmp = json.loads(line)
    #print (tmp.keys())
...
def shallow_parse(text, language='hin', mode='ssf'):
    ...
    for sent in sentences:
        print (sent)
        if sent+'$###$'+language not in _in_hist:
            print ('\t NO')
            _sentences.append(sent)
        else:
            print ('\t YES')

    print ('INNNNNNNNNNN--',sentences)
    sent_count = 0
    output = []
    output_list = []
    #for sentence in sentences:
    if len(_sentences)>0:
        histf = open('nov_12_2022_history.json','a')
        sent_count = sent_count + 1
        ...
        for sentence in _sentences:
            ...
            _in_hist[sentence+'$###$'+language]=p_all[sentence]

            histf.write(json.dumps({sentence+'$###$'+language:p_all[sentence]})+'\n')
    ...
    for sent in sentences:
        output_list.append(_in_hist[sent+'$###$'+language])
